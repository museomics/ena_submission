...
9. Writes the manifest file based on assembly type (**'mitochondrial complete', 'mitochondrial partial', 'mitochondrial contig', 'ribosomal RNA complete','ribosomal RNA partial'**). **Mitochondrial complete/partial** manifests include STUDY, DESCRIPTION, SAMPLE, ASSEMBLYNAME, COVERAGE, PROGRAM (hardcoded as GetOrganelle), PLATFORM, ASSEMBLYTYPE, FASTA, FLATFILE (EMBL annotation flatfile) and CHROMOSOME_LIST. **Ribosomal RNA complete/ribosomal RNA partial** manifests include STUDY, DESCRIPTION, NAME, FASTA and FLATFILE (EMBL annotation flatfile). **Mitochondrial contigs** manifests include STUDY, DESCRIPTION, SAMPLE, ASSEMBLYNAME, PROGRAM, PLATFORM, ASSEMBLYTYPE, FASTA AND FLATFILE. (**NOTE: FASTA is currently hashed out**) 
